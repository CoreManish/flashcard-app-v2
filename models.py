...
class User(db.Model):
    ...
    decks = db.relationship('Deck', backref='decktouser')
    ...
    # d.decktouser.id
    # d.decktouser.name
    cards = db.relationship('Card', backref='cardtouser')
...
class Deck(db.Model):
    ...
    cards = db.relationship('Card', backref='cardtodeck')
...
db.session.add(User(username="manish", password="12345",
               name="Manish Yadav", email="manish@gmail.com"))
db.session.add(User(username="John", password="123456",
...
db.session.add(Card(question="what is PCA", answer='Principal component analysis', user_id="1", deck_id='1'))
db.session.add(Card(question="What is Diagonal Matrix", answer='Only diagonal nonzero', user_id="1", deck_id='1'))
db.session.add(Card(question="what is awk", answer='a programming language', user_id="1", deck_id='2'))
db.session.add(Card(question="Liquid metal", answer='Hg', user_id="2", deck_id='3'))
db.session.add(Card(question="Best book of algebra", answer='Gilbert', user_id="2", deck_id='4'))
db.session.commit()


# During data insertion, foreign key doesn't work with sqlite
"""